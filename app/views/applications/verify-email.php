...
class EmailVerificationView {
    ...
    public function render($data) {
        ...
        <head>
            ...
            <title>Email Verification - FCT College of Nursing Sciences</title>
        ...
        <body>
            <div class="verify-modern-wrap">
                ...
                <div class="verify-modern-card">
                    ...
                    <div class="verify-modern-body">
                        <div class="verify-modern-greeting">
                            ...
                            <h4>Welcome, <?php echo $this->e($applicant_name ?? 'Applicant'); ?>!</h4>
                ...
                <div class="verify-modern-card">
                    ...
                    <div class="verify-modern-body">
                        <div class="alert-modern danger">
                            ...
                            <span><?php echo $this->e($error); ?></span>
                        </div>
                ...
                <div class="verify-modern-card">
                    ...
                    <div class="verify-modern-body">
                        <div class="alert-modern info">
                            <i class="fas fa-info-circle"></i>
                            <span><?php echo $this->e($message); ?></span>
                        </div>
                ...
                <div class="support-modern">
                    ...
                    <a href="mailto:support@fctcns.edu.ng" rel="noopener noreferrer">support@fctcns.edu.ng</a>
            ...
            // ======================================================
            // Email Verification JavaScript with Security Enhancements
            // ======================================================
            ...
            document.addEventListener('DOMContentLoaded', function() {
                ...
                buttons.forEach(button => {
                    button.addEventListener('click', function(e) {
                        if (this.classList.contains('btn-modern-primary') && !this.hasAttribute('data-no-loading')) {
                            ...
                            const originalSpan = this.querySelector('span') ? this.querySelector('span').textContent : '';
                            
                            this.innerHTML = '<span class="spinner-modern" style="margin-right: 0.5rem;"></span> Loading...';
                            this.style.pointerEvents = 'none';
                            
                            // Track click with CSRF protection
                            if (getCsrfToken()) {
                                fetch('/api/track-verification', {
                                    method: 'POST',
                                    headers: {
                                    ...
                                    body: JSON.stringify({
                                        action: 'verification_button_click',
                                        button_text: originalSpan || 'unknown',
                                        timestamp: Date.now()
                                    })
                                }).catch(() => {}); // Silent fail
                            }
                ...
                // Track resend verification clicks
                const resendLinks = document.querySelectorAll('a[href*="resend-verification"]');
                resendLinks.forEach(link => {
                    link.addEventListener('click', function(e) {
                        if (getCsrfToken()) {
                            fetch('/api/track-resend', {
                                method: 'POST',
                                headers: {
                                    'Content-Type': 'application/json',
                                    'X-CSRF-TOKEN': getCsrfToken()
                                ...
                                body: JSON.stringify({
                                    action: 'resend_verification_click',
                                    timestamp: Date.now()
                                })
                            }).catch(() => {});
                        }
...
// =========================================================
$view = new EmailVerificationView();
