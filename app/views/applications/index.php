...
// Include security helper (this is safe and doesn't break existing code)
require_once APP_PATH . '/helpers/SecurityHelper.php';

// Get security tokens
$csp_nonce = SecurityHelper::getCspNonce();
$csrf_token = SecurityHelper::getCsrfToken();

// Extract data as before (this is the original approach that works)
extract($data ?? []);
...
$currency     = SecurityHelper::e($settings['key_value']['application_currency'] ?? '₦');
$fee          = number_format($settings['key_value']['application_fee']          ?? 2200);
$duration     = SecurityHelper::e($settings['key_value']['program_duration']      ?? '4 Years (2 Yrs ND + 2 Yrs HND)');
$min_score    = SecurityHelper::e($settings['key_value']['min_utme_score']       ?? 170);
$max_sittings = SecurityHelper::e($settings['key_value']['max_olevel_sittings']  ?? 2);
$min_age      = SecurityHelper::e($settings['key_value']['min_age']              ?? 16);

$ph1          = SecurityHelper::e($settings['key_value']['support_phone_1']   ?? '07039837749');
$ph2          = SecurityHelper::e($settings['key_value']['support_phone_2']   ?? '08036625119');
$wa           = SecurityHelper::e($settings['key_value']['support_whatsapp']  ?? '08082775076');
$email        = SecurityHelper::e($settings['key_value']['support_email']     ?? 'support.consap@fcthhss.abj.gov.ng');
$hours        = SecurityHelper::e($settings['key_value']['support_hours']     ?? 'Mon–Fri, 9AM–5PM');

$isOpen       = isset($portal_open) && $portal_open;
$wa_clean     = preg_replace('/[^0-9]/', '', $wa);

// Get SRI hashes for CDN resources
$fontAwesomeSri = SecurityHelper::getSriHash('https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.4.0/css/all.min.css');
$googleFontSri  = SecurityHelper::getSriHash('https://fonts.googleapis.com/css2?family=Inter:wght@300;400;500;600;700&family=Playfair+Display:wght@400;500;600;700&display=swap');
?>
<!DOCTYPE html>
<html lang="en">
<head>
    ...
    <!-- Security Meta Tags -->
    <?php echo SecurityHelper::getSecurityMetaTags(); ?>
    
    <title>FCT College of Nursing Sciences - Admissions Portal</title>

    <!-- CSRF Token for JavaScript -->
    <meta name="csrf-token" content="<?php echo SecurityHelper::getCsrfToken(); ?>">

    <!-- Preconnect for performance -->
    <link rel="preconnect" href="https://fonts.googleapis.com">
    <link rel="preconnect" href="https://fonts.gstatic.com" crossorigin>
    
    <!-- Fonts with SRI hashes -->
    <link href="https://fonts.googleapis.com/css2?family=Inter:wght@300;400;500;600;700&family=Playfair+Display:wght@400;500;600;700&display=swap" 
          rel="stylesheet"
          integrity="<?php echo $googleFontSri; ?>" 
          crossorigin="anonymous">
    
    <!-- Font Awesome with SRI hash -->
    <link rel="stylesheet" 
          href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.4.0/css/all.min.css"
          integrity="<?php echo $fontAwesomeSri; ?>"
          crossorigin="anonymous" 
