...
class NominalRollModel {
    ...
    /**
     * Get all employees with pagination and filters - UPDATED VERSION
     */
    public function getAllEmployees($page = 1, $limit = 5, $filters = []) {
        try {
            ...
            error_log("=== MODEL getAllEmployees DEBUG ===");
            ...
            if (!empty($filters['search'])) {
                ...
                $whereConditions[] = "(e.employee_number LIKE :search1 OR 
                                    e.surname LIKE :search2 OR 
                                    e.first_name LIKE :search3 OR 
            ...
            // Sort order
            $sortBy = isset($filters['sort_by']) ? $filters['sort_by'] : 'surname';
            ...
            $orderBy = "ORDER BY e.`$sortBy` $sortOrder";
            ...
            error_log("Total records: $total, Total pages: $totalPages");
            
            // Get paginated data - CRITICAL: Named placeholders for LIMIT/OFFSET
            $dataSql = "SELECT e.*, 
                        u1.username as created_by_name,
                        u2.username as updated_by_name
                        FROM " . self::TABLE_EMPLOYEES . " e
            ...
            // CRITICAL: Bind LIMIT and OFFSET as integers with NAMED PLACEHOLDERS
            $dataStmt->bindValue(':limit', $limit, PDO::PARAM_INT);
            $dataStmt->bindValue(':offset', $offset, PDO::PARAM_INT);
            
            error_log("Binding LIMIT: $limit, OFFSET: $offset");
            
            $dataStmt->execute();
            $employees = $dataStmt->fetchAll(PDO::FETCH_ASSOC);
            
            error_log("Fetched " . count($employees) . " employees");
            ...
            error_log("Returning result with " . count($employees) . " employees");
            error_log("=== END getAllEmployees DEBUG ===");
    ...
    /**
     * Debug database state - check tables and data
     */
    public function debugDatabase() {
        ...
        try {
            ...
            $tables = ['nominal_roll_employees', 'users', 'user_permissions'];
            
            foreach ($tables as $table) {
                $stmt = $this->db->prepare("SHOW TABLES LIKE ?");
                $stmt->execute([$table]);
                $debug[$table] = $stmt->rowCount() > 0 ? 'EXISTS' : 'MISSING';
                
                if ($stmt->rowCount() > 0) {
                    // Count rows
                    $countStmt = $this->db->query("SELECT COUNT(*) as count FROM $table");
                    $debug[$table . '_count'] = $countStmt->fetch(PDO::FETCH_ASSOC)['count'];
                }
            }
            ...
            if ($userId) {
                ...
                $stmt->execute([$userId]);
                $debug['user_permissions'] = $stmt->fetchAll(PDO::FETCH_COLUMN);
            }
            
            // Check a sample of employees
            $stmt = $this->db->query("SELECT id, employee_number, surname, first_name FROM nominal_roll_employees LIMIT 5");
            $debug['sample_employees'] = $stmt->fetchAll(PDO::FETCH_ASSOC);
